  ...
  <v-container>
    <v-row>
      <v-col cols="2">
        <v-switch
          v-model="subscribed"
          label="Show Subscribed Communities"
        ></v-switch>
      </v-col>
      <v-col cols="6">
      ...
      <template v-for="c in filteredcommunities">
        <v-list-item :key="c.id" v-if="subscribed ? c.your_follow : true">
          <v-list-item-action v-if="$store.state.Username">
            ...
            >
            <v-btn v-else @click="unsubscribe(c.id)"
              ><v-icon left>mdi-trash-can</v-icon>Unsubscribe</v-btn
            >
          </v-list-item-action>
          <v-list-item-content>
            <v-list-item-title
              ><router-link :to="'/c/' + c.id + '/' + c.name">{{
                c.name
              }}</router-link>
              - {{ c.description }}</v-list-item-title
            >
    ...
    <v-list v-else> <h2>Loading...</h2></v-list>
  </v-container>
</template>
<script>
export default {
  ...
  beforeMount: function () {
    if (this.$store.state.Username)
      this.$http
        .get(this.$LOTIDE + "/unstable/communities?include_your=true")
        .then(this.gotCommunities);
    else
      this.$http
        .get(this.$LOTIDE + "/unstable/communities")
        .then(this.gotCommunities);
  },
  methods: {
    ...
    subscribe: function (i) {
      ...
      this.$http
        .post(this.$LOTIDE + "/unstable/communities/" + i + "/follow", postdata)
    ...
    unsubscribe: function (i) {
      ...
      this.$http
        .post(
          this.$LOTIDE + "/unstable/communities/" + i + "/unfollow",
          postdata
        )
