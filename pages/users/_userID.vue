  ...
  methods: {
    ...
    submitChangePassword () {
      if (this.verifyPassword !== this.newPassword) {
        this.$store.dispatch("message/showError", {message:"New passwords don't match"});
        return;
      }
      if (this.newPassword.length < 6) {
        this.$store.dispatch("message/showError", {message:"New password must be at least 6 characters"});
        return;
      }
      const m = {};
